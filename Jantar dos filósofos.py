...
# Variável para o tempo de início
start_time = None

# Lock para garantir que as mensagens sejam impressas sem interferência
print_lock = threading.Lock()

def log_message(message):
    """Função para imprimir mensagens com o tempo desde o início."""
    with print_lock:
...
class Filosofo(threading.Thread):
    ...
    def pensar(self):
        ...
        log_message(f"Filósofo {self.indice} está pensando por {tempo_pensando:.2f} segundos.")
        time.sleep(tempo_pensando)  # Simula o tempo de pensar

    def ficar_com_fome(self):
        """Filósofo fica com fome."""
        log_message(f"Filósofo {self.indice} está com fome.")

    def comer(self):
        """Simula o ato de comer."""
        tempo_comendo = random.uniform(1, 5)  # Tempo aleatório entre 1 e 5 segundos
        log_message(f"Filósofo {self.indice} começa a comer por {tempo_comendo:.2f} segundos.")
        time.sleep(tempo_comendo)  # Simula o tempo de comer
        self.ultimo_tempo_comendo = time.time()  # Atualiza o tempo da última refeição
        log_message(f"Filósofo {self.indice} termina de comer.")
        ...
        if tempo_sem_comer > 10 and not self.prioritario:
            ...
            with lock_fila:
                if self.indice not in fila_prioridade:
                    ...
                    log_message(f"Filósofo {self.indice} entrou na fila de prioridade.")

        if tempo_sem_comer > self.tempo_maximo_inanicao:
            log_message(f"Filósofo {self.indice} faleceu de fome! Não come há {tempo_sem_comer:.2f} segundos.")
            ...
            log_message(f"Filósofo {self.indice} foi reiniciado após falecer.")
        ...
        if pegou_primeiro:
            try:
                ...
                if pegou_segundo:
                    ...
                    log_message(f"Filósofo {self.indice} não conseguiu pegar o segundo garfo.")
            finally:
                primeiro_garfo.release()
        else:
            log_message(f"Filósofo {self.indice} não conseguiu pegar o primeiro garfo.")
        return False
